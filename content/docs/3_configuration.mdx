...
| `IMGFORGE_PROMETHEUS_BIND` | unset          | Optional dedicated metrics listener (e.g., `0.0.0.0:9600`). When unset, metrics remain on the main listener under `/metrics`. See [Prometheus Monitoring](11_prometheus_monitoring). |
...
| Variable                          | Default    | Description & tips                                                                                                                                                  |
|-----------------------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `IMGFORGE_KEY`                    | _required_ | Hex-encoded HMAC key. The decoded byte string is used to sign URLs (see [URL Structure](4_url_structure)). Minimum 32 bytes recommended.                    |
...
Caching is optional but highly recommended for hot content. Enable it via `IMGFORGE_CACHE_TYPE` and allied variables. Full guidance lives in [Cache Configuration](7_caching). At a glance:
...
- **Systemd**: Place variables in `/etc/imgforge.env` and reference them via `EnvironmentFile=` in the unit. See [Deployment](10_deployment/10.2_deployment_manual).
